...
new_func = """function generateReport() {
    let t = "ECODOPPLER DE VASOS DE CUELLO\\n\\n";
    ...
    if ((dGrado === 'normal' || dGrado === 'leve') && (iGrado === 'normal' || iGrado === 'leve')) {
        ...
        if (dPlaca === 'oclusion') {
            ...
            textoHemo += `- Eje DERECHO: Velocidades patológicas (VPS ${dVsp} cm/s, Ratio ${dRatio}) con aliasing y turbulencia.\\n`;
        } else {
        ...
        if (iPlaca === 'oclusion') {
            ...
            textoHemo += `- Eje IZQUIERDO: Velocidades patológicas (VPS ${iVsp} cm/s, Ratio ${iRatio}) con aliasing y turbulencia.\\n`;
        } else {
